...
class Master extends Component {
  ...
  async getBalance() {
    const fUSDCxBal = await this.state.fUSDCx.methods
      .balanceOf(this.state.account)
      .call({ from: this.state.account });
    const adjustedfUSDCx = Number(
      new BigNumber(fUSDCxBal).shiftedBy(-18)
    ).toFixed(5);
  ...
  toggleNetwork = async (isArbitrum) => {
    // Logic to handle network switching
    if (isArbitrum) {
      // Switch to Arbitrum network
      const arbitrumChainId = 42161; 
      await this.switchChain(arbitrumChainId);
    } else {
      // Switch to Goerli network
      const goerliChainId = 5; 
      await this.switchChain(goerliChainId);
    }
  };
    ...
    return (
      <div>
        ...
        {this.state.connected && (
          <Row>
            <Container>
              <NetworkSwitch toggleNetwork={this.toggleNetwork} />
            </Container>
          </Row>
        )}
